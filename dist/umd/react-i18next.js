(function (global, factory) {
  ...
  function Trans(_ref) {
    ...
    let contents = components || children; // Replace any component values with <n></n> placeholders
    // e.g. { br: <br/> } => <0></0>
    ...
    if (values) {
      ...
      Object.keys(values).forEach(valueKey => {
        ...
        const isElement = React__default.isValidElement(_value);

        if (isElement) {
          const valueIndex = contents.length;
          const clonedElement = React__default.cloneElement(_value, _objectSpread({}, _value.props, {
            key: valueIndex
          }), _value.children);
    ...
    return React__default.createElement(useAsParent, additionalProps, renderNodes(contents, translation, i18n));
  }
